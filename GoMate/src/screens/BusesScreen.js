...
const BUSES_DATA = ALL_ROUTES.routes.filter(r => ['Bus', 'Shuttle', 'City Bus'].includes(r.type));
            ...
            <ImageBackground source={{ uri: route.image }} style={styles.routeImage} imageStyle={{ borderRadius: 16 }}>
                <View style={styles.routeCardOverlay}>
                    <TouchableOpacity onPress={handleFavorite} style={styles.favoriteButton}>
                        <Ionicons name={isFavorite ? 'heart' : 'heart-outline'} size={22} color={isFavorite ? themeColors.error : '#fff'} />
                    </TouchableOpacity>
                    <View>
                        <Text style={styles.routeName} numberOfLines={2}>{route.name}</Text>
                        <View style={styles.routeCardFooter}>
                            <View style={[styles.statusContainer, {backgroundColor: route.status === 'Popular' ? themeColors.accent : themeColors.success}]}>
                                <Text style={[styles.statusText, {color: '#fff'}]}>{route.status}</Text>
                            </View>
  ...
  return (
    <SafeAreaView style={[styles.container, {backgroundColor: themeColors.background}]}>
      <View style={[styles.header, {borderBottomColor: themeColors.border}]}>
        ...
        <Text style={[styles.headerTitle, {color: themeColors.text}]}>Bus Routes</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <RouteCard route={item} onPress={() => navigation.navigate('BusDetails', { bus: item })} />}
